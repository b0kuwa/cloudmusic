  ...
  <div id="container">
    ...
    <header class="flex justify-between items-center">
      ...
      <el-popover placement="bottom-end" trigger="click" v-model="popoverVisible">
        <div>
          <el-tag @click.prevent="handleChangeFilter()" size="mini" type="danger" class="cursor-default"
            >全部歌单</el-tag
          >
        ...
        <el-button slot="reference" size="mini" round plain>{{ filter }}</el-button>
      </el-popover>
    </header>
    <!-- 内容展示 -->
    ...
    <main class="grid grid-cols-3 gap-4 overflow-auto" v-infinite-scroll="load">
      ...
      >
        ...
        <div class="mx-2 flex flex-col justify-center text-xs whitespace-nowrap overflow-ellipsis overflow-hidden">
          ...
          <div class="text-gray-400 my-4 overflow-ellipsis">
            <el-tag type="danger" size="mini" effect="plain">{{ item.tag }}</el-tag>
            <span class="overflow-hidden overflow-ellipsis"> {{ item.copywriter }}</span>
          </div>
        </div>
      </router-link>
    </main>
...
export default {
  ...
  methods: {
    ...
    async getPlaylistHighqualityTags() {
      const res = await this.$api.getPlaylistHighqualityTags()
      if (res.code !== 200) {
    ...
    async getTopPlayListHighquality() {
      const res = await this.$api.getTopPlayListHighquality(this.query)
      if (res.code !== 200) {
