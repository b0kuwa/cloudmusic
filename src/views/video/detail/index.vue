			...
			<el-col :span="16">
				...
				<section class="mv-info">
					<div class="flex items-center">
						<img src="" alt="" />
						<span v-for="item in mvdata.artists" :key="item.id">{{
							item.name
						}}</span>
					</div>
					<div class="cursor-default">
						...
						<p
							class="text-xs my-2 transition-all overflow-hidden"
							:class="isExpand ? 'max-h-20' : 'max-h-0'"
						>
							{{ mvdata.desc }}
						</p>
						<p class="my-6">
						...
						<div>
							<h2 class="font-medium">
								听友评论<span class="text-xs text-gray-600 px-2"
									>(已有{{ mvDetailInfo.commentCount }}条评论)</span
								>
							</h2>
						</div>
					</div>
					...
					<ul>
						<li v-for="item in hotComments" :key="item.commentId" class="flex">
							...
							<div class="flex flex-col ml-2 flex-1">
								<div class="text-sm">
									<span class="text-blue-500">{{ item.user.nickname }}:</span
									>{{ item.content }}
								</div>
								<div class="flex-1">
			...
			<el-col :span="8">
				...
				<ul>
					...
					>
						<img
							:src="item.coverUrl"
							class="rounded-md object-cover related-album"
						/>
						<div class="ml-2">
							<h3 class="text-sm">{{ item.title }}</h3>
							<p class="text-gray-500 text-xs text-2-ellipsis">
								by
								<span v-for="author in item.creator" :key="author.userId">{{
									author.userName
								}}</span>
							</p>
						</div>
...
export default {
	...
	methods: {
		...
		handleCurrentChange(currentPage) {
			...
			this.queryInfo = {
				...this.queryInfo,
				offset: (this.currentPage - 1) * this.queryInfo.limit
			}
			this.getNewComment()
		},
		// 初始化数据
